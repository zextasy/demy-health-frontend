...
>
    ...
    <div
        class="relative content-center w-full p-1 text-left bg-white border rounded-md sm:text-sm sm:leading-5 form-control"
    ...
    >
        ...
        @isset($attributes['multiple'])
            <span class="flex flex-wrap space-x-1" x-cloak x-show="selected.length > 0">
                <template x-for="(key, index) in selected" :key="index">
                    <span class="text-gray-800 rounded-full truncate bg-blue-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
                        <span class="px-2 truncate" x-text="data[key]"></span>
                        <span x-show="!disabled" x-bind:class="{'cursor-pointer':!disabled}" class="w-4" @click.prevent.stop="deselectOption(index)"><svg xmlns="http://www.w3.org/2000/svg" height="10px" viewBox="0 0 24 24" class = 'h-4 fill-current'><path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 16.538l-4.592-4.548 4.546-4.587-1.416-1.403-4.545 4.589-4.588-4.543-1.405 1.405 4.593 4.552-4.547 4.592 1.405 1.405 4.555-4.596 4.591 4.55 1.403-1.416z"/></svg></span>
                    </span>
                </template>
            </span>
        @else
            <span class="flex flex-wrap" x-cloak x-show="selected">
                <span class="text-gray-800 rounded-full truncate bg-blue-300 px-2 py-0.5 my-0.5 flex flex-row items-center">
                    <span class="px-2 truncate" x-text="data[selected]"></span>
                    <span x-show="!disabled" x-bind:class="{'cursor-pointer':!disabled}" class="h-4" @click.prevent.stop="deselectOption()"><svg xmlns="http://www.w3.org/2000/svg" height="10px" viewBox="0 0 24 24" class = 'h-4 fill-current'><path d="M12 2c5.514 0 10 4.486 10 10s-4.486 10-10 10-10-4.486-10-10 4.486-10 10-10zm0-2c-6.627 0-12 5.373-12 12s5.373 12 12 12 12-5.373 12-12-5.373-12-12-12zm6 16.538l-4.592-4.548 4.546-4.587-1.416-1.403-4.545 4.589-4.588-4.543-1.405 1.405 4.593 4.552-4.547 4.592 1.405 1.405 4.555-4.596 4.591 4.55 1.403-1.416z"/></svg></span>
                </span>
            </span>
        @endif
            ...
            <div x-ref="dropdown" class="relative z-30 p-2 overflow-y-auto max-h-60" >
                ...
                <template x-for="(key, index) in Object.keys(options)" :key="index" >
                    @isset($attributes['multiple'])
                        <div
                            class="px-2 py-1"
                            x-bind:class="{'bg-gray-300 text-blue hover:none':selected.includes(key), 'hover:bg-blue-500 hover:text-blue cursor-pointer':!(selected.includes(key)), 'bg-blue-500 text-blue':currentIndex==index}"
                    ...
                    @else
                        <div
                            class="px-2 py-1"
                            x-bind:class="{'bg-gray-300 text-blue hover:none':selected==key, 'hover:bg-blue-500 hover:text-blue cursor-pointer':!(selected==key), 'bg-blue-500 text-blue':currentIndex==index}"
                            @click.prevent.stop="selectOption(key)"
                            x-text="Object.values(options)[index]">
                        </div>
                    @endisset
