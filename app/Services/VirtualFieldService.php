    ...
    public function getFilamentFormFields(VirtualFieldableContract $virtualFieldsContract):array
    {
        ...
        foreach ($fields as $field) {
            ...
            $filamentFields[] = match ($value) {
                FieldTypeEnum::BOOLEAN_RADIO->value => Radio::make($field->name)->label($field->label)->boolean(),
                ...
                FieldTypeEnum::DECIMAL->value => TextInput::make($field->name)->label($field->label)->numeric()->mask(fn (TextInput\Mask $mask) => $mask->numeric()->decimalPlaces(2)),
